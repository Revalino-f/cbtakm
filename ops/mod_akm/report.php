	...
	if ($i == $n) {
		echo "
			<div class='page'>
				<table width='100%'>
					<tr>
						<td width='100'><img src='$homeurl/dist/img/diknas.png' height='75'></td>
						<td>
							<CENTER>
								<strong class='f12'>
				...
				<tr height=40px>
					...
					<th>Nama</th>
					<th width='7%'>NPG</th>
					<th width='7%'>NPG Multi</th>
					<th width='7%'>NPG BS</th>
					<th width='7%'>NJodoh</th>
		...
		while ($siswa = mysqli_fetch_array($ckck)) {
			$no++;
			$lama = $jawaban = $skor =$skor_multi=$skor_bs=$skor_urut =$totalskor = $skor_esai = $keterangan = '--';
			...
			if ($nilaiC <> 0) {
				if ($nilai['ujian_mulai'] <> '' and $nilai['ujian_selesai'] <> '') {
					...
					$skor_multi = number_format($nilai['skor_multi'], 2, '.', '');
					$skor_bs = number_format($nilai['skor_bs'], 2, '.', '');
					$skor_urut = number_format($nilai['skor_urut'], 2, '.', '');
					$skor_esai = number_format($nilai['skor_esai'], 2, '.', '');
			...
			echo "
						<tr>
							...
							<td align='center'>$skor_multi</td>
							<td align='center'>$skor_bs</td>
							<td align='center'>$skor_urut</td>
							<td align='center'>$skor_esai</td>
